...
def create_empty_project():
    try:
        ...
        db.session.rollback()
        logger.error(f'Error creating empty project: {e}')
        return jsonify({
            'success': False,
            'error': 'ცარიელი პროექტის შექმნა ვერ მოხერხდა'
        }), 500
...
def add_project_photos(project_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error adding photos to project {project_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'ფოტოების დამატება ვერ მოხერხდა'
        }), 500
...
def update_project_main_image(project_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error updating main image for project {project_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'მთავარი ფოტოს განახლება ვერ მოხერხდა'
        }), 500
...
def delete_project_main_image(project_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error deleting main image for project {project_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'მთავარი ფოტოს წაშლა ვერ მოხერხდა'
        }), 500
...
def delete_project_photo_by_url(project_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error deleting photo from project {project_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'ფოტოს წაშლა ვერ მოხერხდა'
        }), 500
...
def like_project(project_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error liking project {project_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'მოწონება ვერ მოხერხდა'
        }), 500
...
def contact_form():
    try:
        ...
        logger.info(f"Contact form submission from {sender_email}")
...
def get_carousel_images():
    try:
        ...
        return jsonify({
            'success': False,
            'error': 'კარუსელის ფოტოების ჩატვირთვა ვერ მოხერხდა'
        }), 500
...
def add_carousel_image():
    try:
        ...
        db.session.rollback()
        logger.error(f'Error adding carousel image: {e}')
        return jsonify({
            'success': False,
            'error': 'კარუსელის ფოტოს დამატება ვერ მოხერხდა'
        }), 500
...
def update_carousel_image_order(image_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error updating carousel image order {image_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'კარუსელის რიგის განახლება ვერ მოხერხდა'
        }), 500
...
def toggle_carousel_image_status(image_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error toggling carousel image status {image_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'კარუსელის სტატუსის შეცვლა ვერ მოხერხდა'
        }), 500
...
def delete_carousel_image(image_id):
    try:
        ...
        db.session.rollback()
        logger.error(f'Error deleting carousel image {image_id}: {e}')
        return jsonify({
            'success': False,
            'error': 'კარუსელის ფოტოს წაშლა ვერ მოხერხდა'
        }), 500
